...
class Simple_Forum_Helper_Topic extends Mage_Core_Helper_Abstract
{
	...
	public function getOptionsTopics($status_all = true, $skip_id = 0, $where = array(), $no_text = false, $skip_parent = false, $ret_as_it = false, $use_store = false, $store_id= 0, $no_empty = false)
	{
		$ret            = array();
		if(!$no_empty)
		{
	...
	private function addTopicsRecurcive($arr, $ret, $depth = 0)
	{
		if(is_array($arr) && count($arr) > 0)
		{
			foreach($arr as $val)
			{
				...
				$nbsp = str_pad('', 6*($depth/10), '&nbsp;', STR_PAD_LEFT);
				$ret[] = array
				(
					'value'    => $val['topic_id'],
					'label'    => $val['title'],
